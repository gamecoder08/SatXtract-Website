...
import warnings
warnings.filterwarnings("ignore", category=UserWarning)  # To avoid unnecessary warnings
...
def _spline_window(window_size, power=2):
    ...
    wind_outer = (abs(2 * (triang(window_size))) ** power) / 2  # ✅ Fixed
    wind_outer[intersection:-intersection] = 0

    wind_inner = 1 - (abs(2 * (triang(window_size) - 1)) ** power) / 2  # ✅ Fixed
